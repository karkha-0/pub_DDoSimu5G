...
void Udp::handleUpperPacket(Packet *packet)
{
    ...
    //LTH added this
    //std::cout << "Udp::handleUpperPacket destAddr: " << destAddr << std::endl;
    ...
    {
        // LTH added the debug and the paranteces
        std::cout << "Socket ID: " << sd->sockId << ", Local Address: " << sd->localAddr
                  << ", Remote Address: " << sd->remoteAddr << std::endl;
        std::cout << "Udp::handleUpperPacket destAddr.isUnspecified, destAddr1: " << destAddr << std::endl;
        std::cerr << "Error: Destination address is unspecified in packet from: "
                  << packet->getSenderModule()->getFullPath() << std::endl;

        throw cRuntimeError("Packet has no destination address! Sender: %s", packet->getSenderModule()->getFullPath().c_str());
        // LTH ended

        addressReq->setDestAddress(destAddr = sd->remoteAddr);

        // LTH added this
        std::cout << "Udp::handleUpperPacket destAddr.isUnspecified, destAddr2: " << destAddr << std::endl;
    ...
    if (interfaceReq == nullptr && destAddr.isMulticast()) {
        ...
        // LTH added this
        std::cout << "inet::udp - handleUpperPacket - just after if" << std::endl;
...
void Udp::processICMPv4Error(Packet *packet)
{
    ...
    std::cout << "ICMP error received: type=" << type << " code=" << code
            << " about packet " << localAddr << ":" << localPort << " > "
            << remoteAddr << ":" << remotePort << std::endl;
    if (type == 11 && code == 0) {
        std::cerr<< "Udp header not available, ignoring ICMP error" << std::endl;
        //return;
    }
    // End of LTH
    ...
    // LTH added this for now and commented delete packet;
    if (packet->getOwner() != this) {
        PacketDropDetails details;
        details.setReason(LIFETIME_EXPIRED);
        emit(packetDroppedSignal, packet, &details);
        std::cerr << "Skipping deletion of packet not owned by this module: " << packet->getName() << std::endl;
        throw cRuntimeError("skipping deletion of packet not owned by this module:");
...
void Udp::sendUpErrorIndication(SockDesc *sd, const L3Address& localAddr, ushort localPort, const L3Address& remoteAddr, ushort remotePort)
{
    ...
    //LTH added this
    std::cout << ">>>>>>>>> sendUpErrorIndication"<< std::endl;
