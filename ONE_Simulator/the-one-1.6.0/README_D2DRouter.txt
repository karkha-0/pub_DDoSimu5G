Compiling
---------
You can compile ONE from the source code using the included compile_new.bat
or compile_withWarning.bat scripts. That should work both in Windows and 
Unix/Linux environment with Java 6 JDK or later.

It uses Jackson ver 2.18.2 for stream writing JSON:
jackson-core
jackson-databind
jackson-annotations
...
Run using the configuration file incuded sim_Scenario/D2D_pro_sim.txt using 
one.sh Script

Notes
=====

Some of the reporting would now work due to the way messages are created in 
D2DRouter

Delays for Handshake and Message Construction (Message Preparation delay) are 
added before starting message transmission
