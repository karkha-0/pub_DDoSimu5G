...
MacNodeId Binder::registerNode(cModule *module, RanNodeType type, MacNodeId masterId, bool registerNr)
{
    ...
    //LTH added this
    //if (type == UE)
        //allNodeIds.push_back(macNodeId);
    if (type == UE && masterId != 0) {
        allNodeIds.push_back(macNodeId);
        cellToNodesMap_[masterId].insert(macNodeId);
        //EV << "*****LTH cellToNodesMap_" << cellToNodesMap_ << "****" << endl;
        //EV << "*****LTH masterId" << masterId <<"****" << "macNodeId" << macNodeId << "*****"<< endl;
    }
    //LTH end
...
void Binder::initialize(int stage)
{
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        testMsg_ = registerSignal("testMsg"); // LTH added this
        simNodeIds_ = registerSignal("simNodeIds"); // LTH added this
    }
    ...
    if (stage == inet::INITSTAGE_LAST)
    {
        ...
        // LTH added this - Schedule the post-initialization message
        simtime_t offset = 1; // add 1 sec offset time
        //EV << " LTH print the delay for the offset: " << simTime()+offset << "here it is"<< endl;
        scheduleAt(simTime()+offset, postInitMessage); // LTH added this Adjust the delay as needed
}
...
//    EV << "******** LTHLTH******Binder::addUeCollectorToEnodeB - UE " << endl;
//    auto cellIt = cellToNodesMap_.find(cell);
//    if (cellIt != cellToNodesMap_.end()) {
//        EV << "******** LTH ******Binder::addUeCollectorToEnodeB - UE inside first if" << endl;
//        // Cell exists, add the UE to the cell's vector if not already present
//        if (std::find(cellIt->second.begin(), cellIt->second.end(), ue) == cellIt->second.end()) {
//            EV << "******** LTH ******Binder::addUeCollectorToEnodeB - UE inside second if" << endl;
//            cellIt->second.push_back(ue);
//        }
//    } else {
//        // Cell does not exist, create a new entry with the UE
//        EV << "******** LTH ******Binder::addUeCollectorToEnodeB - UE inside else " << endl;
//        cellToNodesMap_[cell] = std::vector<MacNodeId>{ue};
//    }
//    EV << "******** LTH ******Binder::addUeCollectorToEnodeB - UE " << ue << " added to cell " << cell << std::endl;
    //LTH end
}
