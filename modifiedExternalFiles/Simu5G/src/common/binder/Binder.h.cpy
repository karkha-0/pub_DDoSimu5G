...
//    std::map<MacNodeId, L3Address> x2Address_;
  ...
  protected:
    ...
    //LTH adding this to get delayed statistics
    cMessage* postInitMessage; // LTH added this
    std::map<MacNodeId, std::set<MacNodeId>> cellToNodesMap_; // LTH Added this
    //std::map<MacNodeId, std::vector<MacNodeId>> cellToNodesMap_; // LTH Added this

    std::vector<int> allNodeIds; //LTH added this



    //LTH adding this to get delayed statistics
    omnetpp::simsignal_t testMsg_; // LTH added this
    omnetpp::simsignal_t simNodeIds_; // LTH added this

    virtual void initialize(int stages) override;
    virtual int numInitStages() const override { return inet::NUM_INIT_STAGES; }
    virtual void handleMessage(omnetpp::cMessage *msg) override
    {
        //LTH added all this inside the function
        EV << " LTH I am inside handle"<< endl;
        if (msg == postInitMessage) {
            EV << " LTH I am inside Binder::handleMessage -> postInitMessage"<< endl;
            //emit(testMsg_, 1212121212);
            // Stats on all nodes in the simulation
            for (int NodeId : allNodeIds) {
                emit(simNodeIds_, NodeId);
            }

            // Stats on all initializtion of nodes to cells
            for (const auto& cellNodesPair : cellToNodesMap_) {
                MacNodeId cellId = cellNodesPair.first;
                const std::set<MacNodeId>& nodes = cellNodesPair.second;

                emit(testMsg_, cellId);
                emit(testMsg_, 00);
                for (const auto& nodeId : nodes) {
                    emit(testMsg_, nodeId);
                }
                emit(testMsg_, 00);
            }
...
//                //std::stringstream ss;
//                //emit(testMsg_, cellId);
//                ss << "Cell ID: " << cellNodesPair.first << " | Nodes: ";
//                emit(testMsg_, cellNodesPair.first);
//
//                for (const auto& nodeId : cellNodesPair.second) {
//                    ss << nodeId << " ";
//                    emit(testMsg_, nodeId);
//                }
//
//                // Example of emitting a simple string, adjust as needed for your simulation's signal handling
//                //emit(testMsg_, ss.str().c_str());
//            }
//            EV << "********LTH handleMessage->ss.str() " <<  ss.str() << " ******" << endl;
            delete msg;
        } else {
            // Handle other messages if there are any
        }
        //LTH end
    }
  ...
  public:
    Binder()
    {
        ...
        //LTH added this
        postInitMessage = new cMessage("postInitialization");
        EV << " LTH Just after initialize postInitMessage in Binder()"<< endl;
    ...
    virtual ~Binder()
    {
        ...
        //LTH added this
        if (postInitMessage) {
            EV << "-***********LTH I am inside ~Binder()-> if cond for postInitMessage and try and canceel and delete" << endl;
            //cancelAndDelete(postInitMessage); // LTH added this
        }
    }
