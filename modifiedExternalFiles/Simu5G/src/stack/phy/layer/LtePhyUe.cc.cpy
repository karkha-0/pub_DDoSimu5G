...
//            WATCH(rxAmount_);
    ...
    else if (stage == inet::INITSTAGE_PHYSICAL_LAYER)
    {
        ...
        //emit(servingCell_, (long)nodeId_); // LTH added this - this didn't show NR
...
void LtePhyUe::doHandover()
{
    ...
    // collect stat
    emit(servingCell_, (long)masterId_);
    //emit(servingCell_, (long)nodeId_); // LTH added this - this didn't show with NR
