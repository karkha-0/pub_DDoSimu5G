...
// LTH commented this and created a new the method below
...
// LTH created this to enable dynamic traffic behavior adjustment under runtime
void CbrSender::handleMessage(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
    ...
    {
        // Handle external commands for dynamic updates
        std::string cmd = msg->getName();
        if (cmd == "updateParams")
        {
            std::cout << "CbrSender::handleMessage - Reinitializing parameters." << std::endl;
            EV << "LTH - CbrSender::handleMessage - Reinitializing parameters." << endl;
            // Dynamically fetch new parameter values
            size_ = par("PacketSize");
            sampling_time = par("sampling_time");

            // Update destination address
            //destAddress_ = inet::L3Address(par("destAddress").stringValue());

            std::cout << "LTH - CbrSender::handleMessage - Updated parameters: "
               << "PacketSize=" << size_
               << ", sampling_time=" << sampling_time << std::endl;
            EV << "LTH - CbrSender::handleMessage - Updated parameters: "
                           << "PacketSize=" << size_
                           << ", sampling_time=" << sampling_time << endl;
               //<< ", destAddress=" << destAddress_ << std::endl;

            // Reschedule the selfSource message with updated sampling time
            if (selfSource_->isScheduled()) {
                cancelEvent(selfSource_);
            }

           scheduleAt(simTime() + sampling_time, selfSource_);

           std::cout << "CbrSender::handleMessage - Traffic reinitialized." << std::endl;
           EV << "LTH - CbrSender::handleMessage - Traffic reinitialized." << endl;
        }
